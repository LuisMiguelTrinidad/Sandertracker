document.addEventListener("DOMContentLoaded", function() {
	...
	function generabarralibro(elemento){
		...
		barralibrointerior.style.width=`${Math.max(
			paginasleidaslibro/paginastotaleslibro, 24/barralibroexterior.offsetWidth
		) * 100}%`;
	}

	function generabarrasaga(elemento){
		let paginastotalessaga = Array.from(elemento.querySelectorAll(".paginastotales"))
			.reduce((x, y) => x + Number(y.value), 0);
		let paginasleidassaga = Array.from(elemento.querySelectorAll(".paginasleidas"))
			.reduce((x, y) => x + Number(y.value), 0);
	...
	function adjudicaCookies(elemento){
		let leidasbarratotal = getCookie(elemento.parentElement.previousElementSibling.previousElementSibling.textContent);
		elemento.value = leidasbarratotal===""?0:leidasbarratotal.split("/")[0];
		elemento.parentElement.children[2].value = leidasbarratotal===""?elemento.parentElement.children[2].value:leidasbarratotal.split("/")[1];
	}
});
